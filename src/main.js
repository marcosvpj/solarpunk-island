...
import gameModeManagerInstance from "./gameModes/GameModeManager.js";
...
} from "./ui/ResponsiveUtils.js";
import {
  initializeGameContainers as createGameContainers,
  getContainers,
...
async function initializePixi() {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[PIXI] Some assets failed to load, continuing anyway:",
        error,
      );
    }
...
console.log(
  "[Init] Are they the same object?",
  playerStorage === window.playerStorage,
);
...
EventBus.on("refinery:productionModeChanged", () => {
  if (gameModeManager && gameModeManager.isStoryMode()) {
    console.log(
      "[Main] Refinery production mode changed, checking conditions...",
    );
...
function ensureMinimumResourceType(hexes, resourceType, minCount) {
  ...
  const existing = hexes.filter(
    (hex) => hex.resource?.type === resourceType,
  ).length;
  const needed = Math.max(0, minCount - existing);

  console.log(
    `[Grid] Ensuring minimum ${resourceType}: have ${existing}, need ${minCount}, placing ${needed} more`,
  );

  if (needed > 0) {
    // Determine valid terrain for this resource type
    const validTerrain =
      resourceType === "radioactive_waste" ? "ground" : "grass";
    ...
    if (validHexes.length < needed) {
      console.warn(
        `[Grid] Not enough valid hexes for ${resourceType}. Need ${needed}, have ${validHexes.length} valid hexes`,
      );
    }

    // Randomly select and place needed resources
    const selected = shuffleArray(validHexes).slice(
      0,
      Math.min(needed, validHexes.length),
    );
    ...
    console.log(
      `[Grid] Placed ${selected.length} additional ${resourceType} resources`,
    );
  }
}
...
function centerGrid() {
  ...
  console.log(
    `[CenterGrid] Scale: ${currentScale}, Grid center: (${gridCenterX}, ${gridCenterY}), World position: (${worldContainer.position.x}, ${worldContainer.position.y})`,
  );
}
...
function centerHexOnScreen(hex, callback = null) {
  ...
  if (distance < 5) {
    ...
    console.log(
      `[CenterHex] Hex (${hex.q}, ${hex.r}) already centered, no animation needed`,
    );
    return;
  }
  ...
  console.log(
    `[CenterHex] Starting smooth animation to center hex (${hex.q}, ${hex.r})`,
  );
}
...
function updateHexCenterAnimation() {
  ...
  const currentX =
    animationStartPos.x +
    (animationTargetPos.x - animationStartPos.x) * easedProgress;
  const currentY =
    animationStartPos.y +
    (animationTargetPos.y - animationStartPos.y) * easedProgress;
  ...
  if (progress >= 1) {
    ...
    console.log(
      `[CenterHex] Animation complete at world position (${worldContainer.position.x}, ${worldContainer.position.y})`,
    );
  }
}
...
function buildDroneNearFactory(factory) {
  ...
  for (const hex of nearbyHexes) {
    if (GameObjectFactory.canPlaceUnit(hex)) {
      ...
      if (drone) {
        ...
        console.log(
          `[Build] Created drone at (${hex.q}, ${hex.r}) from factory at (${factoryHex.q}, ${factoryHex.r})`,
        );
        return;
      }
    }
  ...
  if (drone) {
    gameState.units.push(drone);
    console.log(
      `[Build] Created drone at factory location (${factoryHex.q}, ${factoryHex.r})`,
    );
  } else {
    console.error(
      `[Build] Failed to create drone near factory at (${factoryHex.q}, ${factoryHex.r})`,
    );
  }
}
  ...
  return directions
    .map((dir) =>
      gameState.hexes.find(
        (h) => h.q === hex.q + dir.q && h.r === hex.r + dir.r,
      ),
    )
...
function collectResource(hex) {
  ...
  if (actualCollected > 0) {
    ...
    console.log(
      `[Collect] About to add ${actualCollected} ${hex.resource.type} to storage`,
    );
    ...
    console.log(
      `[Collect] Collected ${storedAmount} ${hex.resource.type} from (${hex.q}, ${hex.r})`,
    );

    // Show collection feedback
    if (storedAmount < actualCollected) {
      console.warn(
        `[Collect] Only stored ${storedAmount}/${actualCollected} due to storage limits`,
      );
    }
  }
}
...
function processTurnEnd() {
  ...
  const fuelConsumption =
    gameState.fuelConsumptionBase +
    buildingCount * gameState.fuelConsumptionPerBuilding;

  console.log(
    `[Turn ${gameState.currentTurn}] Consuming ${fuelConsumption} fuel (${gameState.fuelConsumptionBase} base + ${buildingCount} buildings × ${gameState.fuelConsumptionPerBuilding})`,
  );
  ...
  // Check progression conditions at turn end (before fuel warnings) - Story mode only
  if (
    gameModeManager &&
    gameModeManager.isStoryMode() &&
    progressionManager &&
    gameState.isLevelActive
  ) {
    checkProgressionConditions();
  }
  ...
  console.log(
    `[Turn ${gameState.currentTurn}] Fuel consumed: ${fuelConsumption}, Fuel remaining: ${playerStorage.getFuel()}, Turns remaining: ${turnsRemaining}`,
  );

  // Recalculate turns remaining after production
  const updatedTurnsRemaining =
    playerStorage.getTurnsRemaining(fuelConsumption);

  // Warn player if low on fuel
  if (updatedTurnsRemaining <= 3 && updatedTurnsRemaining > 0) {
    console.warn(
      `[WARNING] Only ${updatedTurnsRemaining} turns of fuel remaining! Build refineries to convert waste to fuel.`,
    );
  }
...
function checkProgressionConditions() {
  ...
  console.log(
    `[Progression] Checking conditions for Level ${gameState.currentLevelId} on turn ${gameState.currentTurn}`,
  );
...
function processRefineryProduction() {
  ...
  refineries.forEach((refinery) => {
    ...
    if (result.produced) {
      ...
      console.log(
        `[Production] Refinery at (${refinery.hex.q}, ${refinery.hex.r}) produced ${result.resourcesProduced} ${result.resourceType}`,
      );
    } else {
      ...
      if (result.reason === "insufficient_waste") {
        console.log(
          `[Production] Refinery at (${refinery.hex.q}, ${refinery.hex.r}) idle - need ${result.needed} waste, have ${result.available}`,
        );
      } else if (result.reason === "inactive") {
        console.log(
          `[Production] Refinery at (${refinery.hex.q}, ${refinery.hex.r}) inactive - no production mode set`,
        );
      }
    }
  });
  ...
  if (totalProduction.activeRefineries > 0) {
    console.log(
      `[Production Summary] ${totalProduction.activeRefineries} active refineries produced:`,
    );
    ...
    if (totalProduction.materials > 0) {
      console.log(
        `[Production Summary] - ${totalProduction.materials} materials`,
      );
    }
    console.log(
      `[Production Summary] - Used ${totalProduction.wasteUsed} radioactive waste`,
    );
  }

  if (totalProduction.inactiveRefineries > 0) {
    console.log(
      `[Production Summary] ${totalProduction.inactiveRefineries} refineries inactive`,
    );
  }
...
function gameLoop(ticker) {
  ...
  if (!gameState.isPaused) {
    ...
    gameState.turnProgress =
      1 - gameState.timeRemaining / gameState.timePerTurn;
    ...
    if (gameState.timeRemaining <= 0) {
      ...
      // Check progression conditions after turn advance - Story mode only
      if (gameModeManager && gameModeManager.isStoryMode()) {
        checkProgressionConditions();
      }
    }
...
function findHexAtPosition(screenX, screenY) {
  ...
  return (
    gameState.hexes.find((hex) => hex.q === axialQ && hex.r === axialR) || null
  );
}
...
async function initGame() {
  ...
  // Start Level 1 automatically - Story mode only
  if (progressionManager && gameModeManager && gameModeManager.isStoryMode()) {
    setTimeout(() => {
