...
export class Unit extends GameObject {
  ...
  setTarget(targetHex) {
    ...
    console.log(
      `[Unit] ${this.type} set target to (${targetHex.q}, ${targetHex.r})`,
    );
  }
  ...
  moveDiscrete() {
    ...
    if (nextHex) {
      ...
      console.log(
        `[Unit] ${this.type} moved from (${fromHex.q}, ${fromHex.r}) to (${nextHex.q}, ${nextHex.r})`,
      );
    }
    ...
    if (this.path.length === 0) {
      ...
      console.log(
        `[Unit] ${this.type} reached destination at (${this.hex.q}, ${this.hex.r})`,
      );
    }
  }
  ...
  moveSmoothly() {
    if (!this.isMovingSmooth && this.path.length > 0) {
      ...
      if (nextHex) {
        ...
        console.log(
          `[Unit] ${this.type} starting smooth movement to (${nextHex.q}, ${nextHex.r})`,
        );
      }
    }
  ...
  getUnitInfo() {
    return {
      ...
      targetHex: this.targetHex
        ? { q: this.targetHex.q, r: this.targetHex.r }
        : null,
      pathLength: this.path.length,
