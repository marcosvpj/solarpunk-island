...
export class ProgressionManager {
  ...
  handleVictory(data) {
    ...
    this.stats.averageCompletionTime =
      this.stats.totalPlayTime / this.stats.totalLevelsCompleted;

    if (
      !this.stats.fastestCompletion ||
      completionTime < this.stats.fastestCompletion.time
    ) {
      this.stats.fastestCompletion = {
        levelId: this.currentLevelId,
        time: completionTime,
    ...
    console.log(
      `[ProgressionManager] LEVEL COMPLETED! Level ${this.currentLevelId} in ${Math.round(completionTime / 1000)}s (${data.turn} turns)`,
    );
  ...
  handleDefeat(data) {
    ...
    console.log(
      `[ProgressionManager] LEVEL FAILED! Level ${this.currentLevelId} on turn ${data.turn}`,
    );
    console.log(
      `[ProgressionManager] Failure reasons:`,
      data.triggeredConditions.map((c) => c.condition.getDescription()),
    );
  ...
  restartCurrentLevel() {
    if (this.currentLevelId) {
      console.log(
        `[ProgressionManager] Restarting Level ${this.currentLevelId}`,
      );
  ...
  advanceToNextLevel() {
    ...
    if (nextLevel && isLevelUnlocked(nextLevel.id)) {
      console.log(
        `[ProgressionManager] Advancing to Level ${nextLevel.id}: ${nextLevel.name}`,
      );
      this.startLevel(nextLevel.id);
    } else {
      console.log(
        "[ProgressionManager] No next level available or not unlocked",
      );
  ...
  resetGameStateForLevel() {
    ...
    console.log(
      `[ProgressionManager] Game state reset for Level ${this.currentLevelId}`,
    );
  }
   ...
   */
  isCampaignComplete() {
    const enabledLevels = Object.values(LEVELS).filter(
      (level) => level.enabled,
    );
      ...
      campaignProgress: {
        ...
        percentage: Math.round(
          (this.completedLevels.length /
            Object.values(LEVELS).filter((l) => l.enabled).length) *
            100,
        ),
      },
    };
  ...
  loadSaveData(saveData) {
    ...
    console.log(
      `[ProgressionManager] Loaded progression: Level ${this.currentLevelId}, ${this.completedLevels.length} completed`,
    );
    return true;
  }
