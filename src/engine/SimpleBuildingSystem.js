...
import { BUILDINGS, getBuildingData, calculateBuildingCost, canAffordBuilding } from '../configs/GameData.js';
import EventBus from './EventBus.js';
import GameObjectFactory from './GameObjectFactory.js';
import { addSimplifiedMethods, ensureBuildingProperties } from './BuildingCompatibility.js';
...
export class SimpleBuildingSystem {
    ...
    build(hex, type) {
        ...
        if (this.gameState.buildings && !this.gameState.buildings.includes(building)) {
    ...
    demolish(hex) {
        ...
        console.log(`[Demolish] Demolished ${building.type} at (${hex.q}, ${hex.r})`);
    ...
    canUpgrade(building) {
        ...
        return canAffordBuilding(building.type, building.level + 1, this.playerStorage);
    }
    ...
    upgrade(building) {
        ...
        console.log(`[Upgrade] Upgraded ${building.type} to level ${building.level}`);
    ...
    getTooltip(building) {
        ...
        if (building.getTooltipInfo && typeof building.getTooltipInfo === 'function') {
    ...
    getContextMenu(building) {
        console.log(`[SimpleBuildingSystem] Getting context menu for ${building.type}`);
        console.log(`[SimpleBuildingSystem] Building has getContextMenuItems method: ${building.getContextMenuItems && typeof building.getContextMenuItems === 'function'}`);
        ...
        if (building.getContextMenuItems && typeof building.getContextMenuItems === 'function') {
            console.log(`[SimpleBuildingSystem] Calling ${building.type}.getContextMenuItems()`);
            const existing = building.getContextMenuItems();
            console.log(`[SimpleBuildingSystem] ${building.type} getContextMenuItems returned:`, existing);
            if (existing && existing.length > 0) {
                ...
                console.log(`[SimpleBuildingSystem] Enhanced ${building.type} menu with demolish option, total items: ${existing.length}`);
                return existing;
            } else {
                console.log(`[SimpleBuildingSystem] ${building.type} getContextMenuItems returned empty or null, falling back to standard menu`);
            }
        } else {
            console.log(`[SimpleBuildingSystem] ${building.type} does not have getContextMenuItems method, using fallback`);
        }
        ...
        if (this.canUpgrade(building)) {
            ...
            menu.push({
                ...
                disabled: !canAffordBuilding(building.type, building.level + 1, this.playerStorage)
            });
        }
