...
import {
  ...
  BuildingActiveCondition,
  ConsecutiveTurnsCondition,
} from "./BuildingConditions.js";
import {
  SurvivalCondition,
  FuelDepletionCondition,
  TurnLimitCondition,
} from "./SurvivalConditions.js";
...
export class ConditionManager {
  ...
  loadLevelConditions(levelConfig) {
      ...
      levelConfig.winConditions.forEach((config, index) => {
        try {
          ...
          if (condition) {
            ...
            console.log(
              `[ConditionManager] Loaded win condition ${index + 1}:`,
              condition.getDescription(),
            );
          }
        } catch (error) {
          console.error(
            `[ConditionManager] Failed to load win condition ${index + 1}:`,
            error,
          );
        }
      });
      ...
      levelConfig.loseConditions.forEach((config, index) => {
        try {
          ...
          if (condition) {
            ...
            console.log(
              `[ConditionManager] Loaded lose condition ${index + 1}:`,
              condition.getDescription(),
            );
          }
        } catch (error) {
          console.error(
            `[ConditionManager] Failed to load lose condition ${index + 1}:`,
            error,
          );
        }
      });
    ...
    console.log(
      `[ConditionManager] Loaded ${this.winConditions.length} win conditions and ${this.loseConditions.length} lose conditions`,
    );
  }
  ...
  createCondition(config) {
    if (!config.type) {
      console.error(
        "[ConditionManager] Condition config missing type:",
        config,
      );
      return null;
    }
    ...
    if (!conditionClassName) {
      console.error(
        `[ConditionManager] Unknown condition type: ${config.type}`,
      );
      return null;
    }
    ...
    if (!ConditionClass) {
      console.error(
        `[ConditionManager] Condition class not registered: ${conditionClassName}`,
      );
      return null;
    }
  ...
  checkAllConditions() {
    ...
    const allWinConditionsMet =
      winResults.length > 0 && winResults.every((result) => result.result);
  ...
  onDefeatTriggered(results) {
    const triggeredConditions = results.loseConditions.filter(
      (result) => result.result,
    );
    console.log(
      `[ConditionManager] DEFEAT triggered on turn ${results.turn} by:`,
  ...
  getStatus() {
    return {
      ...
      winProgress: this.lastWinCheck
        ? 1.0
        : this.calculateWinProgress(
