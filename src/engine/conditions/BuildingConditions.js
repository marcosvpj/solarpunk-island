...
export class BuildingCountCondition extends BaseCondition {
    validateConfig() {
        ...
        if (this.config.min === undefined && this.config.max === undefined && this.config.exact === undefined) {
            throw new Error('BuildingCountCondition requires min, max, or exact count');
        }
    }
...
export class BuildingActiveCondition extends BaseCondition {
    ...
    getCheckData() {
        const allBuildings = this.gameState.buildings.filter(b => b.type === this.config.building);
        const activeBuildings = allBuildings.filter(b => this.isBuildingActive(b));
...
export class ConsecutiveTurnsCondition extends BaseCondition {
    ...
    validateConfig() {
        ...
        if (!this.config.turns || this.config.turns < 1) {
            throw new Error('ConsecutiveTurnsCondition requires positive turns count');
        }
    ...
    evaluate() {
        ...
        console.log(`[ConsecutiveTurnsCondition] Turn ${currentTurn}: requirements met = ${requirementsMet}`);
        ...
        if (requirementsMet) {
            ...
            if (this.lastMetTurn === null) {
                ...
                console.log(`[ConsecutiveTurnsCondition] Starting new sequence: ${this.consecutiveCount}/${this.config.turns}`);
            } else if (this.lastMetTurn === currentTurn - 1) {
                // Consecutive turn - requirements were met last turn too
                this.consecutiveCount++;
                console.log(`[ConsecutiveTurnsCondition] Consecutive turn! Count now: ${this.consecutiveCount}/${this.config.turns}`);
            } else if (this.lastMetTurn < currentTurn - 1) {
                // Sequence was broken, restart
                this.consecutiveCount = 1;
                console.log(`[ConsecutiveTurnsCondition] Sequence was broken, restarting: ${this.consecutiveCount}/${this.config.turns}`);
            }
            ...
            if (this.consecutiveCount > 0) {
                console.log(`[ConsecutiveTurnsCondition] Sequence broken! Resetting from ${this.consecutiveCount} to 0`);
            }
            this.consecutiveCount = 0;
            this.lastMetTurn = null;
        }
        ...
        console.log(`[ConsecutiveTurnsCondition] Condition ${isComplete ? 'COMPLETED' : 'not complete'}: ${this.consecutiveCount}/${this.config.turns} consecutive turns`);
    ...
    checkRequirements() {
        return this.config.requirements.every(requirement => {
            switch (requirement) {
                ...
                default:
                    console.warn(`[ConsecutiveTurnsCondition] Unknown requirement: ${requirement}`);
                    return false;
            }
        });
    }
    ...
    checkBothRefineriesOperational() {
        const refineries = this.gameState.buildings.filter(b => b.type === 'refinery');
