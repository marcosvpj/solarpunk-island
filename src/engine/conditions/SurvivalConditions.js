...
export class SurvivalCondition extends BaseCondition {
    ...
    evaluate() {
        ...
        if (this.config.requirements) {
            return hasBasicResources && this.config.requirements.every(req => this.checkRequirement(req));
        }
    ...
    getCheckData() {
        return {
            ...
            requirements: this.config.requirements?.map(req => ({
                requirement: req,
...
export class FuelDepletionCondition extends BaseCondition {
    ...
    getCheckData() {
        const fuel = this.playerStorage.getFuel();
        const fuelConsumption = this.gameState.fuelConsumptionBase + 
                              (this.gameState.buildings.length * this.gameState.fuelConsumptionPerBuilding);
    ...
    onFirstMet() {
        super.onFirstMet();
        console.log(`[FuelDepletionCondition] CIVILIZATION COLLAPSE: Fuel depleted on turn ${this.gameState.currentTurn}`);
    }
}
...
export class TurnLimitCondition extends BaseCondition {
    ...
    getCheckData() {
        return {
            ...
            turnsRemaining: Math.max(0, this.config.maxTurns - this.gameState.currentTurn),
    ...
    onFirstMet() {
        super.onFirstMet();
        console.log(`[TurnLimitCondition] TIME LIMIT EXCEEDED: Turn ${this.gameState.currentTurn} > ${this.config.maxTurns}`);
    }
}
...
export class StorageExceededCondition extends BaseCondition {
    ...
    evaluate() {
        ...
        switch (resource) {
            ...
            case 'waste':
                ...
                break;
            default:
                console.warn(`[StorageExceededCondition] Unknown resource: ${resource}`);
                return false;
        }
    ...
    onFirstMet() {
        super.onFirstMet();
        const data = this.getCheckData();
        console.log(`[StorageExceededCondition] STORAGE LIMIT EXCEEDED: ${data.resource} = ${data.currentAmount} > ${data.limit}`);
    }
}
...
export class ResourceDepletionCondition extends BaseCondition {
    ...
    evaluate() {
        ...
        switch (resource) {
            ...
            case 'waste':
                ...
                break;
            default:
                console.warn(`[ResourceDepletionCondition] Unknown resource: ${resource}`);
                return false;
        }
    ...
    onFirstMet() {
        super.onFirstMet();
        const data = this.getCheckData();
        console.log(`[ResourceDepletionCondition] RESOURCE DEPLETED: ${data.resource} reached zero`);
    }
}