...
export class SimpleGameManager {
    ...
    async init(containers) {
        ...
        this.buildingSystem = new SimpleBuildingSystem(this.state, this.playerStorage);
    ...
    handleHexClick(hex) {
        ...
        const screenPos = { x: this.app.screen.width / 2, y: this.app.screen.height / 2 };
    ...
    gameLoop(ticker) {
        ...
        this.state.turnProgress = 1 - (this.state.timeRemaining / GAME_BALANCE.turn.duration);
    ...
    processTurnEnd() {
        ...
        const fuelConsumption = GAME_BALANCE.turn.baseFuelConsumption + 
        ...
        const turnsRemaining = Math.floor(this.playerStorage.getFuel() / fuelConsumption);
