...
export class ScreenManager {
  ...
  async showScreen(screenName, options = {}) {
    if (this.transitioning) {
      console.warn(
        `[ScreenManager] Already transitioning, ignoring request for ${screenName}`,
      );
      return;
    }
    ...
    const transitionType =
      options.transition || config.transition || SCREEN_TRANSITIONS.FADE;
    const duration =
      options.duration ||
    ...
    console.log(
      `[ScreenManager] Showing screen: ${screenName} with transition: ${transitionType}`,
    );
