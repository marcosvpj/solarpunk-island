...
async function testSimplifiedSystems() {
  ...
  try {
    const { getBuildingData, calculateBuildingCost, BUILDINGS } = await import(
      "./configs/GameData.js"
    );
  ...
  try {
    const { longTomorrowManager } = await import(
      "./systems/longTomorrow/index.js"
    );
    console.log("✅ Long Tomorrow systems loaded");
    console.log(
      "  - Research trees available:",
      Object.keys(longTomorrowManager.researchPointManager.researchProgress),
    );
  } catch (error) {
    console.log(
      "⚠️  Long Tomorrow systems not loaded (this is OK if not using Long Tomorrow mode)",
    );
  }
