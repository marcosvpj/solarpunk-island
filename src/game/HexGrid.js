...
export class HexGrid {
  ...
  createHexRing(ring) {
    ...
    console.log(
      `[HexGrid] Created ring ${ring} with ${ringHexes.length} hexes`,
    );
    return ringHexes;
  }
  ...
  expandToRadius(newRadius) {
    if (newRadius <= this.currentRadius) {
      console.log(
        `[HexGrid] Already at radius ${this.currentRadius}, no expansion needed`,
      );
      return;
    }

    console.log(
      `[HexGrid] Expanding from radius ${this.currentRadius} to ${newRadius}`,
    );
    ...
    console.log(
      `[HexGrid] Expansion complete. Grid now has ${this.hexes.length} hexes`,
    );
  }
  ...
  distributeResources(resourceCounts) {
    const availableHexes = this.hexes.filter(
      (hex) => hex.q !== 0 || hex.r !== 0,
    );
  ...
  distributeResourcesToHexes(hexes, resourceCounts = null) {
    ...
    const wasteHexes = hexes.filter(
      (hex) => hex.terrain === "ground" && !hex.resource,
    );
    const shuffledWasteHexes = this.shuffleArray(wasteHexes);
    for (
      let i = 0;
      i < Math.min(resourceCounts.radioactive_waste, shuffledWasteHexes.length);
      i++
    ) {
    ...
    // Distribute forest
    const forestHexes = hexes.filter(
      (hex) => hex.terrain === "grass" && !hex.resource,
    );
    const shuffledForestHexes = this.shuffleArray(forestHexes);
    for (
      let i = 0;
      i < Math.min(resourceCounts.forest, shuffledForestHexes.length);
      i++
    ) {
