...
export class Habitat extends Building {
  constructor(hex) {
    ...
    if (!habitatConfig) {
      console.error(
        "[Habitat] Could not find habitat configuration in GameData.js",
      );
      return;
    }
  ...
  upgrade() {
    ...
    if (playerStorage.getMaterials() < this.upgradeCost) {
      console.log(
        `[Habitat] Insufficient materials for upgrade (need ${this.upgradeCost}, have ${playerStorage.getMaterials()})`,
      );
      return false;
    }
    ...
    if (success) {
      ...
      console.log(
        `[Habitat] Upgraded to level ${this.level} - Housing: ${this.getHousingCapacity()}, Comfort: ${this.getComfortLevel()}`,
      );
    }
  ...
  destroy() {
    ...
    console.log(
      `[Habitat] Habitat destroyed - Housing lost: ${this.getHousingCapacity()}`,
    );
  }
}
