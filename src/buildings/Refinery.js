...
export class Refinery extends Building {
  constructor(hex) {
    ...
    console.log(
      `[Refinery] Created refinery at (${hex.q}, ${hex.r}) - Production mode: ${this.productionMode}`,
    );
  }
  ...
  convertToFuel() {
    ...
    if (wasteAvailable < ratio.input) {
      console.log(
        `[Refinery] Insufficient waste for fuel conversion (need ${ratio.input}, have ${wasteAvailable})`,
      );
      return false;
    }

    // Perform conversion
    const wasteRemoved = playerStorage.removeResources(
      ratio.input,
      "radioactive_waste",
    );
    if (wasteRemoved === ratio.input) {
      ...
      console.log(
        `[Refinery] Converted ${wasteRemoved} waste to ${fuelAdded} fuel`,
      );
      return true;
    }
  ...
  convertToMaterials() {
    ...
    if (wasteAvailable < ratio.input) {
      console.log(
        `[Refinery] Insufficient waste for materials conversion (need ${ratio.input}, have ${wasteAvailable})`,
      );
      return false;
    }
    ...
    if (wasteRemoved === ratio.input) {
      const materialsAdded = playerStorage.addResources(
        ratio.output,
        "materials",
      );
      ...
      console.log(
        `[Refinery] Converted ${wasteRemoved} waste to ${materialsAdded} materials`,
      );
      return true;
    }
  ...
  canConvertToFuel() {
    ...
    return (
      playerStorage &&
      playerStorage.getWaste() >= this.conversionRatios.fuel.input
    );
  }
  ...
  canConvertToMaterials() {
    ...
    return (
      playerStorage &&
      playerStorage.getWaste() >= this.conversionRatios.materials.input
    );
  }
  ...
  processProduction() {
    ...
    if (wasteRemoved === ratio.input) {
      ...
      const resourcesAdded = playerStorage.addResources(
        ratio.output,
        resourceType,
      );
