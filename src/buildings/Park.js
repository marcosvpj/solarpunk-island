...
export class Park extends Building {
  ...
  upgrade() {
    ...
    if (playerStorage.getMaterials() < this.upgradeCost) {
      console.log(
        `[Park] Insufficient materials for upgrade (need ${this.upgradeCost}, have ${playerStorage.getMaterials()})`,
      );
      return false;
    }
    ...
    if (success) {
      ...
      console.log(
        `[Park] Upgraded to level ${this.level} - Comfort: ${this.getComfortLevel()}`,
      );
    }
