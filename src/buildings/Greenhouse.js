...
export class Greenhouse extends Building {
    ...
    getTooltipInfo() {
        ...
        const productionStatus = this.canProduce() ? '✓ Producing' : '⚠ Need water';
    ...
    processProduction() {
        ...
        // Add food to storage (assuming food is a resource type)
        const actualAmount = playerStorage.addResources(foodProduced, 'food');
        ...
        if (actualAmount > 0) {
            ...
            console.log(`[Greenhouse] Produced ${actualAmount} food at (${this.hex.q}, ${this.hex.r})`);
    ...
    upgrade() {
        ...
        if (playerStorage.getMaterials() < this.upgradeCost) {
            console.log(`[Greenhouse] Insufficient materials for upgrade (need ${this.upgradeCost}, have ${playerStorage.getMaterials()})`);
            return false;
        }
        ...
        if (success) {
            ...
            this.upgradeCost = Math.floor(this.baseUpgradeCost * Math.pow(1.6, this.level - 1));
            ...
            console.log(`[Greenhouse] Upgraded to level ${this.level} - Food: ${this.getFoodProduction()}/turn, Water: ${this.getWaterConsumption()}/turn`);
        }
    ...
    destroy() {
        ...
        console.log(`[Greenhouse] Greenhouse destroyed - Food production lost: ${this.getFoodProduction()}/turn`);
    }
}