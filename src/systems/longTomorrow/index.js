...
export { WorldStateManager, ResearchPointManager, AchievementSystem };

// Export constants
export { RESEARCH_TREES, ACHIEVEMENTS, ACHIEVEMENT_CATEGORIES };
...
export class LongTomorrowManager {
  ...
  initializeSession(options = {}) {
    ...
    const worldState = this.worldStateManager.initializeWorldState(
      options.previousWorldState,
    );
  ...
  processSessionCompletion(gameState, sessionData = {}) {
    console.log(
      "[LongTomorrowManager] Processing session completion for world evolution",
    );

    // Record current session state
    const sessionState = this.worldStateManager.recordSessionState(
      gameState,
      sessionData,
    );
    ...
    return {
      ...
      achievementStats,
      newAchievements,
      worldEvolutionSummary: this.generateEvolutionSummary(
        sessionState,
        newWorldState,
      ),
    };
  ...
  generateEvolutionSummary(sessionState, newWorldState) {
    const summary = {
      forestsGrown:
        ...
        (sessionState.worldState?.stats.forestsGrown || 0),
  ...
  getResearchTreeStatus() {
    ...
    return {
      ...
      primarySpecialization:
        this.researchPointManager.getPrimarySpecialization(),
    };
  }
