...
export class WorldStateManager {
    constructor() {
        ...
        this.transformationRules = {
            ...
            buildingDecay: {
                basic: { recoveryRate: 0.50, decayRate: 0.0 }, // 50% materials, no decay
                advanced: { recoveryRate: 0.75, decayRate: 0.10 }, // 75% + rare materials, 10% decay
                highLevel: { becomesRuin: true, hasChoices: true } // Level 3+ become interactive ruins
            },
            soilMemory: {
                enriched: { bonus: 0.25, maxBonus: 0.50, duration: 3 },
                depleted: { penalty: 0.50, recoveryTime: 2 },
    ...
    recordSessionState(gameState, sessionData = {}) {
        ...
        console.log('[WorldStateManager] Session state recorded:', sessionState.sessionId);
    ...
    evolveWorldState(previousSession) {
        console.log('[WorldStateManager] Evolving world state from session:', previousSession.sessionId);
    ...
    applyTerrainEvolution(newWorldState, previousSession) {
        ...
        if (previousGameState.terrain) {
            previousGameState.terrain.forEach((terrainData, hexId) => {
                const rule = this.transformationRules.terrainEvolution[terrainData.type];
    ...
    recordBuildingLifecycles(gameState) {
        return {
            ...
            finalBuildingCount: (gameState.buildings && gameState.buildings.size) || 0
        };
    }
