...
export class AchievementSystem {
  constructor(researchPointManager = null) {
    ...
    console.log(
      "[AchievementSystem] Initialized with",
  ...
  setupEventListeners() {
    ...
    EventBus.on(
      ...
      this.handleBuildingConstructed.bind(this),
    );
  ...
  handlePopulationChanged(data) {
    ...
    this.sessionStats.peakPopulation = Math.max(
      this.sessionStats.peakPopulation,
      data.population,
    );
    this.sessionStats.lowPopulation = Math.min(
      this.sessionStats.lowPopulation,
      data.population,
    );
  ...
  handleSessionEnded(data) {
    console.log(
      "[AchievementSystem] Session ended with",
      this.sessionAchievements.length,
      "new achievements",
    );
  ...
  unlockAchievement(achievementId) {
    ...
    if (this.researchPointManager) {
      this.researchPointManager.awardResearchPoints(
        achievement.id,
        achievement.points,
      );
    }
  ...
  checkAchievementProgress() {
    ...
    // Utopian Society
    if (
      this.sessionStats.population >= 100 &&
      this.sessionStats.happiness >= 1.0
    ) {
    ...
    // Green Paradise
    if (
      this.sessionStats.population >= 50 &&
      this.sessionStats.renewableEnergyOnly
    ) {
  ...
  loadSaveData(saveData) {
    ...
    if (saveData.sessionStats) {
      this.sessionStats = {
        ...
        ...saveData.sessionStats,
      };
      // Restore Sets
      if (saveData.sessionStats.buildingTypesBuilt) {
        this.sessionStats.buildingTypesBuilt = new Set(
          saveData.sessionStats.buildingTypesBuilt,
        );
      }
      if (saveData.sessionStats.allLuxuryTypes) {
        this.sessionStats.allLuxuryTypes = new Set(
          saveData.sessionStats.allLuxuryTypes,
        );
      }
    }

    console.log(
      "[AchievementSystem] Loaded save data:",
